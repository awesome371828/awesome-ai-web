#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AWESOME AI WEB — копия DeepSeek: память диалога, оптимизация, файлы"""
...
SEARCH_TIMEOUT = 4
WEATHER_TIMEOUT = 3
MAX_HISTORY = 20  # сколько прошлых сообщений ИИ помнит
...
def init_db():
    ...
    try:
        cur.execute("ALTER TABLE messages_web ADD COLUMN IF NOT EXISTS image TEXT"); conn.commit()
...
def can_send_message(user_id):
    if int(user_id) == OWNER_ID or is_admin(user_id): return True
    if is_banned(user_id): return False
    if get_premium_status(user_id): return True
    conn = get_db(); cur = conn.cursor()
...
def get_chat_history(chat_id):
    """Возвращает последние сообщения диалога для контекста (память ИИ)"""
...
def generate_with_gigachat(history, system_prompt):
    """history - список {role, content} - передаём ИИ всю память диалога"""
    try:
        token = get_gigachat_token()
        if not token: return None
        messages = [{"role": "system", "content": system_prompt[:1800]}]
        messages += [{"role": h["role"], "content": (h.get("content") or "")[:500]} for h in history if h.get("role") in ("user", "assistant")]
        r = requests.post(
            "https://gigachat.devices.sberbank.ru/api/v1/chat/completions",
            headers={"Authorization": f"Bearer {token}", "Content-Type": "application/json", "Accept": "application/json"},
            json={"model": "GigaChat-Pro", "messages": messages, "temperature": 0.9, "max_tokens": 2000},
...
Ты ПОМНИШЬ весь диалог. Отвечай с учётом предыдущих сообщений пользователя — это важно.
ПРАВИЛА: развёрнутый полезный ответ, структура, списки, примеры, цифры, эмодзи (🔥🧠💡⚡🚀). Сложное разбивай на шаги. НЕ извиняйся, НЕ повторяй вопрос, НЕ пиши "возможно/наверное". Ты живой и эмоциональный.
    ...
    try:
        ...
        r = requests.post(
            ...
            json={"model": "GigaChat-Pro",
                  "messages": [{"role": "system", "content": "Ты — ИИ, видишь изображения. Подробно опиши объекты, действия, текст, цвета. На русском."},
                               {"role": "user", "content": [{"type": "text", "text": "Что на этом изображении?"},
...
def process_message(user_id, user_text, history, image_desc=None):
    """history - весь диалог (память). image_desc - если прислано фото"""
    tl = user_text.lower().strip()

    # Фото: строим промпт с учётом истории
    if image_desc:
        ...
        sp += "\n\nОтветь, учитывая изображение и запрос. Если запрос пустой — подробно опиши фото."
        if get_premium_status(user_id): sp += "\n\n💎 PREMIUM режим."
        hist = history + [{"role": "user", "content": user_text or "Опиши изображение"}]
        answer = generate_with_gigachat(hist, sp)
    ...
    # Передаём ИИ всю историю диалога + новый вопрос
    answer = generate_with_gigachat(history + [{"role": "user", "content": user_text}], sp)
    if answer and len(answer) > 5:
        ...
        if check and "ПОДТВЕРЖДАЮ" not in check.upper():
            fixed = generate_with_gigachat(history + [{"role":"user","content":f"Исправь ошибки: {check}\nМой ответ:\n{answer}"}], "Исправь ответ с учётом замечаний.")
...
def api_login():
    d = request.json
    uid = str(d.get('user_id','')).strip(); name = str(d.get('username','')).strip() or 'unknown'
    if not uid.isdigit(): return jsonify({'ok':False,'error':'Введите корректный Telegram ID'})
    ensure_user(int(uid), name); session['user_id'] = int(uid); session['username'] = name
...
def api_chat():
    ...
    # Загружаем историю ДО добавления нового сообщения (память)
    ...
    response = process_message(uid, msg, history, image_desc)
...
/* ЛЁГКИЙ фон (без тяжёлого blur — для плавности на ПК) */
.bg{position:fixed;inset:0;z-index:-2;background:linear-gradient(160deg,#0f1117 0%,#151a2a 50%,#0f1117 100%);transition:background .3s}
...
.new-chat{margin:14px;padding:12px;background:linear-gradient(135deg,var(--accent),var(--accent2));border:none;border-radius:12px;color:#fff;font-weight:600;cursor:pointer;font-size:14px;transition:transform .15s}
...
.sugg{background:var(--panel);border:1px solid var(--border);border-radius:14px;padding:16px;cursor:pointer;transition:transform .15s;font-size:13px}
...
.overlay{position:fixed;inset:0;background:rgba(15,17,23,.92);z-index:100;display:flex;align-items:center;justify-content:center}
...
.toast{position:fixed;top:20px;right:20px;background:var(--panel);border:1px solid var(--border);border-radius:12px;padding:14px 20px;z-index:200;box-shadow:0 8px 30px rgba(0,0,0,.4);max-width:320px}
...
if __name__ == '__main__':
    print("=" * 60)
    print("🧠 AWESOME AI WEB — память диалога + оптимизация")
    print("=" * 60)
    print(f"✅ ИИ помнит до {MAX_HISTORY} прошлых сообщений чата")
    print("✅ Удаление чата на крестик")
    print("✅ Оптимизированный интерфейс (без тормозов на ПК)")
    print("=" * 60)
